...
#################################################
#             Push Notification 91 days         #
#################################################
#
# 1. Select all mobile number where expiry day + 61 days
# 2. Send notifications to mobile number where svn numbers are on 91st day
# 3. Update status of svn_id (number tag on the mobile number) from EXPIRED to UNASSIGNED
# 4. Update date modified of a specific svn_id
# 5. Update svn_id of a specific mobile number into NULL
# 6. Update users table date_modified
# 7. Insert data to svn_transactions table
#
#########################################################
#               DEFINING VARIABLES                      #
#########################################################
...
def push_notif(mnum, bearer):
	...
	payload = '{"min" :[{"mobileNumber" :' + mnum +'}],"type" : "Notification","click_action" : "NotificationActivity","title": "91st day: Deactivated Philippine Number","body": "Your Philippine Mobile Number subscription is now deactivated. A new mobile number will be assigned to you after subscribing to a new plan."}'
...
def main():
	...
	token1 = get_token()
	with open("/home/ec2-user/pushnotif/pushnotif91/logs.tmp","a+") as afile:
		...
		def overall():
			...
			cur = conn.cursor()
			cur.execute("SELECT a.mobile_number FROM users a LEFT JOIN svn b ON a.svn_id = b.id WHERE current_date - expiry_date::DATE =61 AND b.status='EXPIRED';")
			conn.commit();
			...
			for mobileNum in result: # Mobile number 1 by 1 until all the rows are done
				...
				def updateStatus():
					try:
						queryUpdate=(f"UPDATE svn SET status='UNASSIGNED' WHERE id = (SELECT svn_id FROM users WHERE mobile_number = '{mobileNum}')")
						cursor(queryUpdate)
						afile.write(f'[{currentTime}] Successfully updated svn status from EXPIRED to UNASSIGNED of mobile number {mobileNum} \n')
					except Exception as update_err:
						afile.write(f'[{currentTime}] Unsucessful to update svn status of mobile number {mobileNum} \n')
						afile.write(f'[{currentTime}] Getting error {update_err} on mobile number {mobileNum} \n')

				### Function to update svn_id of a specific mobile number into NULL ###		
				def updateNull():
					try:
						queryUpdate1=(f"UPDATE users SET svn_id = NULL WHERE mobile_number='{mobileNum}';")
						cursor(queryUpdate1)
						afile.write(f'[{currentTime}] Successfully updated svn_id of {mobileNum} into null. \n')
					except Exception as update_err1:
						afile.write(f'[{currentTime}] Unsucessful to update svn_id into null for mobile number {mobileNum} \n')
						afile.write(f'[{currentTime}] Getting error {update_err1} on mobile number {mobileNum} \n')

				### Function to update svn date_modified ###
				def updateDateMod():
					try:
						queryUpdate2=(f"UPDATE svn SET date_modified=NOW() WHERE id = (SELECT svn_id FROM users WHERE mobile_number = '{mobileNum}')")
						cursor(queryUpdate2)
				...
				### Function to update mobile_number date_modified ###
				def updateDateMod2():
					try:
						queryUpdate3=(f"UPDATE users SET date_modified=NOW() WHERE mobile_number='{mobileNum}';")
						cursor(queryUpdate3)
						afile.write(f'[{currentTime}] Successfully updated date modified of mobile number {mobileNum} \n')
					except Exception as update_err3:
						afile.write(f'[{currentTime}] Unsucessful to update date modified of mobile number {mobileNum} \n')
				...
				def insertSvnTrans():
					...
					except Exception as update_err4:
						afile.write(f'[{currentTime}] Unsucessful to update svn transaction of mobile number {mobileNum} \n')
						afile.write(f'[{currentTime}] Getting error {update_err4} on mobile number {mobileNum} \n')
...
def rewrite():
	f = open("/home/ec2-user/pushnotif/pushnotif91/logs.tmp","w+")
