...
# SELECT all ids from svn table counter 86 days
try:
        cur = conn.cursor()
        cur.execute("SELECT svn_id FROM svn WHERE status='ACTIVE'")
#       cur.execute("SELECT id FROM svn WHERE current_date - expiry_date::DATE = 56") 
