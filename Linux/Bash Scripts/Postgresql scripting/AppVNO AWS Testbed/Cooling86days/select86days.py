...
#  This script is to SELECT all ids from users with 86 days counter.
...
# SELECT all mobile numbers from users table using svn number - counter 86 days.
try:
        cur = conn.cursor()
        cur.execute("SELECT a.mobile_number FROM users a LEFT JOIN svn b ON a.svn_id = b.id WHERE current_date - expiry_date::DATE = 56")
# Print results to list.tmp
        result = cur.fetchall()
        list_stdout = sys.stdout
        list = open("list86days.tmp", "w")
        sys.stdout = list
        for row in result:
                print(row[0])
