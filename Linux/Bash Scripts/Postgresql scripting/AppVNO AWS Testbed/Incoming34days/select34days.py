...
#  This script is to SELECT all ids from users with 34 days counter.
...
# SELECT all mobiles number from users table using svn number - counter 34 days.
try:
        cur = conn.cursor()
        cur.execute("SELECT a.mobile_number FROM users a LEFT JOIN svn b ON a.svn_id = b.id WHERE current_date - expiry_date::DATE = 4")
# Print results to list.tmp
        result = cur.fetchall()
        list_stdout = sys.stdout
        list = open("list34days.tmp", "w")
        sys.stdout = list
        for row in result:
                print(row[0])
